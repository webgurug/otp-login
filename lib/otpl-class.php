...
if(!class_exists('CWSOtpLoginFront'))
{
    class CWSOtpLoginFront
    {
        /**
        ...
        public function __construct()
        {
			...
			add_action('wp_footer', array(&$this,'CWS_otpl_popup_html') );
			add_action( 'wp_ajax_nopriv_otplaction', array(&$this, 'CWS_otpl_login_action') );
			//add_action( 'wp_ajax_otplaction', array(&$this, 'CWS_otpl_login_action') );
		 ...
		 */		
		 ...
		 public function CWS_otpl_popup_html(){
			...
			jQuery(document).ready(function() {
				jQuery("#otpl_lightbox .close span").click(function() { 
					//alert("Yesss");
				jQuery("#otpllightbox").html("");jQuery("#otpl_lightbox").hide().fadeOut(1000);});
			...
			//check external url or not
			 ...
			 $otplFormHtml .='</div>';
			 $otplFormHtml  .='<div class="heading"><h3>OTP Verification</h3></div><div id="otpl-body"><div id="sendotp">
			 <label for="email">Please enter your email ID to Login and vote.<span class="req">*</span><span class="emailerror req"></span></label><input type="email" name="email" id="email" value="" class="otpl-req-fields" size="40"> <input type="submit" class="otpl_submit_btn generateOtp" id="generateOtp" value="Next"> </div>
					...
					//End social-inner
				$otplFormHtml.='<script type="text/javascript">
				jQuery(window).load(function(e){
					...
					var content =jQuery("#otpl_contact").html();
							...
							"<div id=\"otpl_lightbox\">" +
								"<div id=\"otpl_content\">" +
								"<div class=\"close\"></div>"  + content  + "</div>" +	"</div>";
		 ...
		 * 
		 * */	 
		public function CWS_otpl_send_otp($email,$otp){
		// send OTP over email
		$otp_message = '<table style="background: #f2f2f2;padding: 50px;text-align: center;" width="650" bgcolor="#fff" align="center">
			<tbody>
				<tr>
					<td>
						<h3 style="font-weight: 400;font-size: 25px;margin: 0 0 50px;color: #222;line-height: 36px;">We have received a one time password request.</h3>
						<h1 style="margin: 0 0 30px;font-size: 28px;color: #66131d;">Your new OTP is <span style="float: left;width: 100%;margin: 30px 0 0;font-size: 60px;color: #226703;"> '.$otp.' </span> </h1> 
					</td>
				</tr>
			 </tbody>
		    </table>';
			...
			//From
			//hook to send html template
			add_filter('wp_mail_content_type', function( $otp_message ) {
            return 'text/html';
			});
		 ...
		 * 
		 * */	 
		public function CWS_otpl_login_action(){
				global $wpdb;
				// check security 
				if(wp_doing_ajax())
			   ...
			   if(empty( $email))
				...
				//check for login
				//$db_login_check = get_user_meta( $user_id, "user_sucess_emilotp",  true);	
				
				if ( ! $user_id && false == email_exists($email) ) 
				{
					echo json_encode(array('status'=>0,'message'=>'You are not registered to vote.','response'=>'You are not registered to vote.'));
					  wp_die();
					...
					//check type of login request
					   // send otp on email address
						...
						$otpmail = $this->CWS_otpl_send_otp($email,$newotp); 
						update_user_meta( $user_id, "emilotp", $newotp); 
						//update first name
						$userOtp = get_user_meta( $user_id, "emilotp");	
					   if(!$otpmail){
					...
					$db_otp = get_user_meta( $user_id, "emilotp",  true);
					 if($db_otp==$email_otp && $validateotp!=0){
						 ...
						 if(is_user_logged_in())
						 {
						  ...
						  update_user_meta( $user_id, "user_sucess_emilotp",  'sucess_login' );// reset otp
						  wp_die();
						 }
...
if(class_exists('CWSOtpLoginFront'))
{
    ...
    $CWSOtpLoginFront = new CWSOtpLoginFront();
}
